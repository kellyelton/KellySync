...
namespace KellySync
{
    public class IOWatcher : IDisposable
    {
        ...
        public IOWatcher( Config config, string path )
            : this( config, path, null, WatcherChangeTypes.All ) {
        }
        public IOWatcher( Config config, string path, string filter )
            : this( config, path, filter, WatcherChangeTypes.All ) {
        }
        public IOWatcher( Config config, string path, WatcherChangeTypes eventsToHandle )
            : this( config, path, null, eventsToHandle ) {
        }
        public IOWatcher( Config config, string path, string filter, WatcherChangeTypes eventsToHandle ) {
            _config = config; // TODO: I really don't like this dependency in here, figure out how to axe it.
        ...
        protected virtual void OnIOEventFired( WatcherChangeTypes eveType, object sender, FileSystemEventArgs e ) {
            ...
            var args = new IOEventArgs( e, _config );
